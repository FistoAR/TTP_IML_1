                  ...
                  {bulkPayment.paymentType === "advance" && bulkPayment.amount && (
                    <div className="flex justify-between text-[1vw]">
                      ...
                      <span className="font-bold text-blue-600">
                        ₹{Math.max(calculateTotals().balance - parseFloat(bulkPayment.amount || 0), 0).toFixed(2)}
                      </span>
                    </div>
                  )}
                </div>
...
const imlNameRefs = useRef({});
  ...
  if (existingOrder) {
    ...
    setOrderEstimate(existingOrder.orderEstimate || { estimatedNumber: '', estimatedValue: 0 }); // NEW
  }
}, [existingOrder]);
  ...
  if (value.trim() === '') {
    ...
    setShowLidColorSuggestions({ ...showLidColorSuggestions, [productId]: false });
    return;
  }
  ...
  setShowLidColorSuggestions({ ...showLidColorSuggestions, [productId]: filtered.length > 0 });
};
...
const handleTubColorInput = (productId, value) => {
  ...
  if (value.trim() === '') {
    ...
    setShowTubColorSuggestions({ ...showTubColorSuggestions, [productId]: false });
    return;
  }
  ...
  setShowTubColorSuggestions({ ...showTubColorSuggestions, [productId]: filtered.length > 0 });
};
  ...
  const handleClickOutside = (event) => {
    ...
    Object.keys(lidColorRefs.current).forEach(productId => {
      if (
        ...
        setShowLidColorSuggestions(prev => ({ ...prev, [productId]: false }));
      }
    });
    ...
    Object.keys(tubColorRefs.current).forEach(productId => {
      if (
        ...
        setShowTubColorSuggestions(prev => ({ ...prev, [productId]: false }));
      }
    });
    ...
    <div className="bg-gray-50 p-0">
      <div className="max-w-[90vw] mx-auto bg-white rounded-[0.8vw] shadow-sm pb-[.75vw]">
        ...
        <div className="px-[1.5vw] max-h-[65vh] overflow-auto">
            ...
            <div className="grid grid-cols-5 gap-[1.5vw]">
             ...
             <div className="grid grid-cols-5 gap-[1.5vw] mt-[1vw]">
                ...
                <Input
                  ...
                  onChange={(e) => setOrderEstimate({...orderEstimate, estimatedNumber: e.target.value})}
                  type="number"
                />
                <Input
                  ...
                  onChange={(e) => setOrderEstimate({...orderEstimate, estimatedValue: e.target.value})}
                  type="number"
                />
                </div>
          </Section>
                      ...
                      <div className="grid grid-cols-4 gap-[1.5vw] mt-[1vw]">
                        ...
                        >
                          ...
                          <input 
                            ...
                            onChange={(e) => handleLidColorInput(product.id, e.target.value)}
                            className="w-full text-[0.85vw] px-[0.75vw] py-[0.45vw] border border-gray-300 bg-white rounded-[0.5vw] outline-none box-border focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                          />
                          
                          {/* Suggestions Dropdown */}
                          {showLidColorSuggestions[product.id] && filteredLidColors.length > 0 && (
                            <div className="absolute z-50 w-full bg-white border border-gray-300 rounded-[0.5vw] mt-[0.25vw] shadow-lg max-h-[12vw] overflow-y-auto">
                              {filteredLidColors.map((color, index) => (
                                <div
                                  key={index}
                                  onClick={() => handleLidColorSelect(product.id, color)}
                                  className="px-[1vw] py-[0.6vw] hover:bg-blue-50 cursor-pointer border-b border-gray-100 last:border-b-0"
                                >
                        ...
                        >
                          ...
                          <input 
                            ...
                            onChange={(e) => handleTubColorInput(product.id, e.target.value)}
                            className="w-full text-[0.85vw] px-[0.75vw] py-[0.45vw] border border-gray-300 bg-white rounded-[0.5vw] outline-none box-border focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                          />
                          
                          {/* Suggestions Dropdown */}
                          {showTubColorSuggestions[product.id] && filteredTubColors.length > 0 && (
                            <div className="absolute z-50 w-full bg-white border border-gray-300 rounded-[0.5vw] mt-[0.25vw] shadow-lg max-h-[12vw] overflow-y-auto">
                              {filteredTubColors.map((color, index) => (
                                <div
                                  key={index}
                                  onClick={() => handleTubColorSelect(product.id, color)}
                                  className="px-[1vw] py-[0.6vw] hover:bg-blue-50 cursor-pointer border-b border-gray-100 last:border-b-0"
                                >
                                  <div className="flex items-center gap-[0.5vw]">
                        ...
                        <div className="grid grid-cols-4 gap-1.5vw mt-1vw">
                          <div>
                            ...
                            <input 
                              ...
                              value={product.lidColor}
                              onChange={(e) => updateProduct(product.id, 'lidColor', e.target.value)}
                          ...
                          <div>
                            ...
                            <input 
                              ...
                              placeholder="Select or type color"
                              value={product.tubColor}
                              onChange={(e) => updateProduct(product.id, 'tubColor', e.target.value)}
                              className="w-full px-0.75vw py-0.45vw border border-gray-300 bg-white rounded-0.5vw text-0.85vw outline-none box-border focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                            />
          ...
          <Section title="Payment Details - Client Payments"   styles={{ position: 'relative' }}>
            {/* Compact Summary Bar */}
