...
export default function OrdersManagement2() {
  ...
  useEffect(() => {
    ...
    if (storedVersion !== DATA_VERSION) {
      ...
      console.log(`Old version: ${storedVersion}, New version: ${DATA_VERSION}`);
      ...
      alert(`Data structure updated to version ${DATA_VERSION}. Old data has been cleared.`);
    }
  }, []);
...
const calculateRemainingLabels = (product) => {
  ...
  if (product.imlType === "LID" || product.imlType === "LID TUB") {
    ...
    console.log(`  LID: ${lidOrdered} ordered - ${lidProduced} produced = ${lidOrdered - lidProduced}`);
  }
  ...
  if (product.imlType === "TUB" || product.imlType === "LID TUB") {
    ...
    console.log(`  TUB: ${tubOrdered} ordered - ${tubProduced} produced = ${tubOrdered - tubProduced}`);
  }
...
const hasRemainingLabels = (order) => {
  ...
  console.log(`  Result: ${hasRemaining ? '✅ HAS' : '❌ NO'} remaining labels`);
  return hasRemaining;
};
  ...
  Object.entries(allGrouped).forEach(([companyName, orders]) => {
    Object.entries(orders).forEach(([orderKey, order]) => {
      ...
      const matchesViewMode =
        viewMode === "all" || hasRemainingLabels(order);
 ...
 const PaymentManagementModal = ({ order, onClose, onSave }) => {
  ...
  // FIXED: Get estimated value from the correct location
    ...
    if (order.orderEstimate?.estimatedValue) {
      console.log("Found estimatedValue in orderEstimate.estimatedValue:", order.orderEstimate.estimatedValue);
      return parseFloat(order.orderEstimate.estimatedValue);
    }
    
    // From dummy data: payment[0].totalEstimated (DUMMY ORDERS)
    if (order.payment && Array.isArray(order.payment) && order.payment[0]?.totalEstimated) {
      console.log("Found estimatedValue in payment[0].totalEstimated:", order.payment[0].totalEstimated);
      return parseFloat(order.payment[0].totalEstimated);
    }
    
    // From payment object: payment.totalEstimated (IF STORED AS OBJECT)
    if (order.payment && !Array.isArray(order.payment) && order.payment.totalEstimated) {
      console.log("Found estimatedValue in payment.totalEstimated:", order.payment.totalEstimated);
  ...
  const addPaymentRecord = () => {
    ...
    const newRecord = {
      ...
      method: bulkPayment.paymentType === "advance" ? bulkPayment.method : "PO",
      amount: bulkPayment.paymentType === "advance" ? parseFloat(bulkPayment.amount) : 0,
    ...
    alert(bulkPayment.paymentType === "advance" ? "Payment recorded successfully!" : "PO recorded successfully!");
  ...
  const deletePaymentRecord = (index) => {
    ...
    const updatedRecords = existingPaymentRecords.filter((_, i) => i !== index);
    const updatedOrder = {
      ...order,
        ...
        <div className="p-[1.5vw]">
          ...
          <div className="mb-[2vw]">
            ...
            <div className="grid grid-cols-2 gap-[1vw] bg-gray-50 p-[1vw] rounded-[0.5vw]">
              <div>
                ...
                <p className="text-[.9vw] font-medium">{order.contact?.company || "N/A"}</p>
              </div>
              <div>
                <p className="text-[0.8vw] text-gray-500">Contact Person</p>
                <p className="text-[.9vw] font-medium">{order.contact?.contactName || "N/A"}</p>
              </div>
              <div>
                <p className="text-[0.8vw] text-gray-500">Phone</p>
                <p className="text-[.9vw] font-medium">{order.contact?.phone || "N/A"}</p>
              </div>
              <div>
                <p className="text-[0.8vw] text-gray-500">IML Name</p>
                <p className="text-[.9vw] font-medium">{order.contact?.imlName || "N/A"}</p>
              </div>
              <div>
                <p className="text-[0.8vw] text-gray-500">Order Number</p>
                <p className="text-[.9vw] font-medium">{order.orderNumber || "N/A"}</p>
              </div>
              <div>
            ...
            <div className="overflow-x-auto rounded-lg">
              <table className="w-full border-collapse text-[0.85vw]">
                <thead className="bg-gray-100">
                  <tr>
                    <th className="border border-gray-300 p-[0.75vw] text-left">Product Name</th>
                    <th className="border border-gray-300 p-[0.75vw] text-left">Size</th>
                    <th className="border border-gray-300 p-[0.75vw] text-left">IML Name</th>
                    <th className="border border-gray-300 p-[0.75vw] text-left">IML Type</th>
                    <th className="border border-gray-300 p-[0.75vw] text-left">LID Qty</th>
                    <th className="border border-gray-300 p-[0.75vw] text-left">TUB Qty</th>
                  ...
                  {order.products && order.products.length > 0 ? (
                    order.products.map((product, idx) => (
                      <tr key={idx} className="border-b">
                        <td className="border border-gray-300 p-[0.75vw]">{product.productName || "N/A"}</td>
                        <td className="border border-gray-300 p-[0.75vw]">{product.size || "N/A"}</td>
                        <td className="border border-gray-300 p-[0.75vw]">{product.imlName || "N/A"}</td>
                        <td className="border border-gray-300 p-[0.75vw]">{product.imlType || "N/A"}</td>
                        <td className="border border-gray-300 p-[0.75vw]">{product.lidLabelQty || "-"}</td>
                        <td className="border border-gray-300 p-[0.75vw]">{product.tubLabelQty || "-"}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan="6" className="p-[1vw] text-center text-gray-500">
                        No products found
                      </td>
                    </tr>
                  )}
          ...
          <div className="mb-[2vw]">
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-[1.5vw] rounded-[0.5vw] border border-blue-200">
                  ...
                  <div className="space-y-[0.5vw]">
                    <div className="flex justify-between text-[1vw]">
                      ...
                      <span className="font-medium">
                        {bulkPayment.paymentType === "advance" ? "Advance Payment" : "Purchase Order"}
                      </span>
                    </div>
                    ...
                    {bulkPayment.paymentType === "advance" && bulkPayment.amount && (
                      <div className="flex justify-between text-[1vw]">
                        ...
                        <span className="font-bold text-blue-600">
                          ₹{Math.max(totals.balance - parseFloat(bulkPayment.amount || 0), 0).toFixed(2)}
                        </span>
                      </div>
                    )}
                  </div>
              ...
              >
                {bulkPayment.paymentType === "advance" ? "💰 Record Payment" : "📄 Record PO"}
              </button>
            </div>
          </div>
          ...
          <div>
              ...
              <div className="overflow-x-auto rounded-lg">
                <table className="w-full text-[0.85vw] border border-gray-200 ">
                  <thead className="bg-gradient-to-r from-gray-100 to-gray-200">
                    <tr>
                      <th className="border border-gray-300 p-[0.75vw] text-left border-b border-gray-300">Date & Time</th>
                      <th className="border border-gray-300 p-[0.75vw] text-left border-b border-gray-300">Type</th>
                      <th className="border border-gray-300 p-[0.75vw] text-left border-b border-gray-300">Method</th>
                      <th className="border border-gray-300 p-[0.75vw] text-left border-b border-gray-300">Amount</th>
                      <th className="border border-gray-300 p-[0.75vw] text-left border-b border-gray-300">Remarks</th>
                      <th className="border border-gray-300 p-[0.75vw] text-center border-b border-gray-300">Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {existingPaymentRecords.map((record, idx) => (
                      <tr key={idx} className="border-b hover:bg-gray-50">
                        ...
                        <td className="border border-gray-300 p-[0.75vw]">
                          <span className={`px-[0.5vw] py-[0.2vw] rounded-[0.3vw] text-[0.8vw] font-medium ${
                            record.paymentType === "advance" 
                              ? "bg-green-100 text-green-700" 
                              : "bg-blue-100 text-blue-700"
                          }`}>
                            {record.paymentType === "advance" ? "Advance" : "PO"}
                          </span>
                        </td>
                        <td className="border border-gray-300 p-[0.75vw]">{record.method}</td>
                        <td className="border border-gray-300 p-[0.75vw] font-medium text-green-600">
                          {record.paymentType === "advance" 
                            ? `₹${record.amount.toLocaleString()}` 
                            : "-"}
                        </td>
                        <td className="border border-gray-300 p-[0.75vw]">{record.remarks || "-"}</td>
            ...
            <div className="mt-[1vw] bg-gradient-to-r from-gray-50 to-blue-50 p-[1.5vw] rounded-[0.5vw] grid grid-cols-3 gap-[1vw] border border-gray-200">
              <div>
                <p className="text-[0.8vw] text-gray-500 mb-[0.3vw]">Total Estimated</p>
                <p className="text-[1.3vw] font-bold text-blue-600">
                  ₹{estimatedValue.toLocaleString()}
                </p>
              </div>
              <div>
                <p className="text-[0.8vw] text-gray-500 mb-[0.3vw]">Total Received</p>
                <p className="text-[1.3vw] font-bold text-green-600">
                  ₹{totals.totalPaid.toLocaleString()}
                </p>
              </div>
              <div>
                <p className="text-[0.8vw] text-gray-500 mb-[0.3vw]">Balance Due</p>
            ...
            <div className="flex items-center-justify-center gap-[1vw]">
              ...
              {viewMode === "remaining" && (
                <div className="flex items-center gap-2 px-[1vw] py-[0.6vw] bg-orange-50 border-l-4 border-orange-500 rounded">
                  <svg className="w-[1.2vw] h-[1.2vw] text-orange-600" fill="currentColor" viewBox="0 0 20 20">
          ...
          {expandedCompanies[companyName] && (
            <div className="space-y-[1.25vw] p-[1vw]">
              {Object.entries(orders).map(([orderKey, order]) => {
                ...
                return (
                  <div key={orderKey} className="bg-gray-50 border border-1 border-gray-400 rounded-lg overflow-hidden">
                    {/* Order Header */}
                    <div
                              ...
                              <thead>
                                <tr className="bg-gray-200">
                                  ...
                                  <th className="border border-gray-300 px-[1.25vw] py-[.75vw] text-left text-[.85vw] font-semibold">
                                   ...
                                   {viewMode === "remaining" ? (
                                      <>
                                        <th className="border border-gray-300 px-[1.25vw] py-[.75vw] text-left text-[.85vw] font-semibold">Total Ordered</th>
                                        <th className="border border-gray-300 px-[1.25vw] py-[.75vw] text-left text-[.85vw] font-semibold">Produced</th>
                                        <th className="border border-gray-300 px-[1.25vw] py-[.75vw] text-left text-[.85vw] font-semibold">Remaining</th>
                                      </>
                                    ) : (
                                      <>
                                        <th className="border border-gray-300 px-[1.25vw] py-[.75vw] text-left text-[.85vw] font-semibold">LID Qty</th>
                                        <th className="border border-gray-300 px-[1.25vw] py-[.75vw] text-left text-[.85vw] font-semibold">TUB Qty</th>
                                      </>
                                    )}
                                  ...
                                  {viewMode === "remaining" && (
                                    <th className="border border-gray-300 px-[1.25vw] py-[.75vw] text-center text-[.85vw] font-semibold">Action</th>
                                  )}
                                </tr>
                              </thead>
                              <tbody>
                                {order.products.map((product, idx) => {
                                    ...
                                    const totalOrdered = product.imlType === "LID" || product.imlType === "LID TUB"
                                      ? parseInt(product.lidLabelQty) || 0
                                      : product.imlType === "TUB"
                                      ? parseInt(product.tubLabelQty) || 0
                                      : 0;
                                    
                                    const totalProduced = product.imlType === "LID" || product.imlType === "LID TUB"
                                      ? parseInt(product.lidProductionQty) || 0
                                      : product.imlType === "TUB"
                                      ? parseInt(product.tubProductionQty) || 0
                                      : 0;
                                        ...
                                        <td className="border border-gray-300 px-[1.25vw] py-[.75vw] text-[.85vw]">
                                          ...
                                          {viewMode === "remaining" && (
                                              <td className="border border-gray-300 px-1.25vw py-.75vw text-center">
                                                  ...
                                                  <button
                                                    onClick={() => handleMoveToProduction(order, product)}
