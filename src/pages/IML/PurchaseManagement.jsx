...
const PurchaseManagement = () => {
  ...
  useEffect(() => {
    ...
    if (storedOrders) {
      ...
      const convertedEntries = convertOrdersToPurchaseEntries(parsedOrders);
      console.log("Converted Entries:", convertedEntries);

      if (convertedEntries.length > 0) {
        console.log("✓ Using converted entries from orders");
        setPurchaseEntries(convertedEntries);
      } else {
        console.log("⚠ No products marked for purchase, checking fallback...");
        const stored = localStorage.getItem(STORAGE_KEY_PURCHASE);
        if (stored) {
          const parsed = JSON.parse(stored);
          if (Array.isArray(parsed) && parsed.length > 0) {
            console.log("✓ Using purchase entries");
      ...
      console.log("⚠ No orders found, checking purchase entries...");
      const stored = localStorage.getItem(STORAGE_KEY_PURCHASE);
      if (stored) {
        const parsed = JSON.parse(stored);
        if (Array.isArray(parsed) && parsed.length > 0) {
  ...
  const filteredEntries = useMemo(() => {
    ...
    if (startDate || endDate) {
      const latestFollowup = getLatestFollowupData(entry.id, activeSheet);
      if (latestFollowup.date) {
        // Parse DD/MM/YYYY format
        const [day, month, year] = latestFollowup.date.split('/');
        const followupDate = new Date(year, month - 1, day);
        
        if (startDate) {
          const start = new Date(startDate);
          start.setHours(0, 0, 0, 0);
          matchesDate = matchesDate && followupDate >= start;
        }
        
        if (endDate) {
          const end = new Date(endDate);
          end.setHours(23, 59, 59, 999);
          matchesDate = matchesDate && followupDate <= end;
        }
      } else {
        // If no followup date exists and filter is active, exclude it
        matchesDate = false;
      }
    }
  ...
  <div className="flex gap-[1vw] items-end">
          ...
          <div className="absolute z-10 w-full mt-[0.3vw] bg-white border border-gray-300 rounded-md shadow-lg max-h-[15vw] overflow-y-auto">
            {filteredSuppliers.map((supplier, index) => (
              <div
                ...
                className="px-[0.7vw] py-[0.6vw] text-[0.9vw] cursor-pointer hover:bg-blue-50 transition-colors border-b border-gray-100 last:border-b-0"
              >
                {supplier}
              </div>
            ))}
    ...
    <div className="flex-none">
      <button
        ...
        className="px-[1.2vw] py-[0.7vw] text-[0.9vw] bg-red-500 text-white rounded-md font-medium hover:bg-red-600 transition-colors cursor-pointer whitespace-nowrap"
      >
        Clear Filters
      </button>
    </div>
